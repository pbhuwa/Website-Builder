<template>
    ...
    >
    <div class="p-2 flex items-center space-x-2 mb-1">
        <input
            v-model="websiteName"
            ...
            required
            :class="{
                'px-3 py-1 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500': true,
                'px-3 py-1 border border-red-500 rounded focus:outline-none focus:ring-2 focus:ring-red-500': hasError
            }"
        />

        <button
            class="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-4 py-1 rounded shadow focus:outline-none focus:ring-2 focus:ring-blue-500"
            @click="saveToWebsite"
        >
            Save
        </button>
        ...
        <button
            v-if="websiteId"
            class="bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-1 rounded shadow focus:outline-none focus:ring-2 focus:ring-green-500"
        ...
        <a
            ...
            target="_blank"
            v-if="publishedUrl"
            class="bg-gray-500 hover:bg-gray-600 text-white font-semibold px-4 py-1 rounded shadow focus:outline-none focus:ring-2 focus:ring-gray-500"
        >
            View Site
        </a>
    </div>
...
<div class="properties-panel" v-if="activeElement">
    ...
    @update="updateActiveElement($event)"
    />
</div>
</div>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            activeElement() {
                return this.activeElementIndex !== null
                ? this.elements[this.activeElementIndex]
                : null;
            },
            canvasStyle() {
                return {
        ...
        methods: {
            getComponentName(type) {
                return type === 'header' ? 'HeaderElement' :
                type === 'homepage' ? 'HomeElement' :
                type === 'services' ? 'ServicesElement' :
                type === 'about-us' ? 'AboutUsElement' :
                type === 'contact' ? 'ContactElement' :
                type === 'footer' ? 'FooterElement' :
                type === 'text-block' ? 'TextBlock' :
                type === 'image' ? 'ImageElement' :
                type === 'button' ? 'ButtonElement' :
                type === 'input' ? 'InputElement' :
                type === 'card' ? 'CardElement' : '';
            },
            ...
            getDefaultProperties(type) {
                const defaults = {
                    ...
                    'about-us': {
                        ...
                        mission: {
                            title: 'Our Mission',
                            content: 'We are dedicated to delivering exceptional value through innovative solutions and unparalleled customer service. Our team of experts works tirelessly to exceed expectations and build lasting relationships with our clients.'
                        },
                        team: {
                            show: true,
                            title: 'Meet Our Team',
